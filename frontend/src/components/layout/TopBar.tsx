...
export default function TopBar({ onToggleSidebar }: Props) {
  ...
  return (
    <header className="h-16 border-b border-white/10 bg-dark flex items-center px-4 gap-4 shrink-0">
      {/* Sidebar toggle */}
      <button
      ...
      <div className="flex-1 max-w-md">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all"
          />
          <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-600 bg-white/5 px-1.5 py-0.5 rounded border border-white/10">⌘K</kbd>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {notifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-dark2 rounded-xl border border-white/10 shadow-2xl z-50 fade-in">
              ...
              <div className="divide-y divide-white/5">
                {mockNotifications.map(n => (
                  <div key={n.id} className={`flex gap-3 p-4 hover:bg-white/5 cursor-pointer ${n.unread ? '' : 'opacity-60'}`}>
              ...
              <div className="p-3 border-t border-white/10">
                <button className="w-full text-xs text-gray-400 hover:text-white text-center transition-colors">View all notifications</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={userMenuRef}>
          ...
          >
            ...
            <span className="text-sm text-white hidden sm:block">{user?.user_metadata?.full_name?.split(' ')[0] || 'User'}</span>
            <ChevronDown className="w-3 h-3 text-gray-500" />
          </button>
          {userMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-dark2 rounded-xl border border-white/10 shadow-2xl z-50 fade-in overflow-hidden">
              <div className="p-3 border-b border-white/10">
                <div className="text-sm font-medium text-white">{user?.user_metadata?.full_name || 'User'}</div>
