...
const decisions = [
  { id: 'approved', label: 'Approve', icon: CheckCircle, color: 'text-green-400', bg: 'bg-green-500/10 border-green-500/30 hover:bg-green-500/20' },
  { id: 'rejected', label: 'Reject', icon: XCircle, color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/30 hover:bg-red-500/20' },
  { id: 'escalated', label: 'Escalate', icon: ArrowUpCircle, color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/30 hover:bg-orange-500/20' },
]
...
export default function ExceptionDecision() {
  ...
  return (
    <div className="fade-in space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-3 gap-3">
          {decisions.map(d => (
            <button key={d.id} onClick={() => setDecision(d.id)}
              className={`flex flex-col items-center gap-2 p-4 rounded-xl border transition-all ${decision === d.id ? d.bg + ' ring-1 ring-offset-1 ring-offset-dark2 ring-primary' : 'bg-white/5 border-white/10 hover:border-white/20'}`}>
      ...
      {decision === 'approved' && (
        <div className="bg-dark2 rounded-xl border border-white/10 p-6 fade-in">
          <label className="block text-sm font-medium text-gray-300 mb-2">Conditions & Limitations (optional)</label>
      ...
      <div className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl">
        ...
        <p className="text-xs text-gray-400">I confirm this decision is based on my review of all available evidence, policies, and precedents. This decision will be permanently recorded.</p>
      </div>
