    ...
    <div className="min-h-screen bg-dark flex flex-col items-center justify-center p-6">
      ...
      <div className="max-w-2xl w-full">
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-sm font-semibold text-white">ExceptionOS Story Demo</span>
        ...
        <div className="bg-dark2 rounded-2xl border border-white/10 p-8 mb-6 fade-in" key={step}>
          <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mb-4">
            {step === steps.length - 1 ? <CheckCircle className="w-7 h-7 text-green-400" /> : <span className="text-2xl font-bold text-primary">{step + 1}</span>}
          </div>
          <h2 className="text-2xl font-bold text-white mb-3">{steps[step].title}</h2>
          <p className="text-gray-300 text-lg leading-relaxed">{steps[step].desc}</p>
        </div>

        <div className="flex gap-3 justify-between">
          <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/10 hover:bg-white/15 text-white font-medium transition-colors border border-white/10 disabled:opacity-30">
