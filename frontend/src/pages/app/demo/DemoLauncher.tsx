...
export default function DemoLauncher() {
  ...
  return (
    <div className="min-h-screen bg-dark flex flex-col items-center justify-center p-6">
      <button onClick={() => navigate('/')} className="absolute top-6 left-6 flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors">
      ...
      <h1 className="text-3xl font-bold text-white mb-3 text-center">ExceptionOS Interactive Demo</h1>
      <p className="text-gray-400 text-center max-w-xl mb-10">Experience the full power of organizational decision intelligence. No account required.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl w-full mb-8">
        {[
          ...
          <button key={d.title} onClick={() => navigate(d.to)}
            className={`p-5 rounded-2xl border text-left hover:border-primary/40 transition-all ${d.featured ? 'bg-primary/10 border-primary/30' : 'bg-dark2 border-white/10'}`}>
