...
export default function PolicyDrift() {
  return (
    <div className="fade-in space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">Policy Drift Analysis</h1>
        <p className="text-gray-400 text-sm">Policies where exception volume signals potential need for policy revision.</p>
      </div>

      <div className="flex items-center gap-3 p-4 bg-red-500/5 border border-red-500/20 rounded-xl">
        <AlertTriangle className="w-5 h-5 text-red-400 shrink-0" />
        <div>
          <div className="text-sm font-semibold text-white">2 policies showing high drift</div>
          <div className="text-xs text-gray-400 mt-0.5">Budget Authorization Policy and IT Procurement Policy are receiving exceptions at 3x the normal rate. Consider policy revision.</div>
        </div>
      </div>
      ...
      <div className="bg-dark2 rounded-xl border border-white/10 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-white">Exception Volume vs. Threshold</h2>
        </div>
