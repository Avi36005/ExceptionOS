  ...
  return (
    <div className="fade-in max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Ask Organizational Memory</h1>
          <p className="text-sm text-gray-400">Query your organization's institutional knowledge in natural language.</p>
        </div>
      </div>

      <div className="bg-dark2 rounded-2xl border border-white/10 flex flex-col" style={{ height: 520 }}>
        {/* Messages */}
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${m.role === 'assistant' ? 'bg-primary/20' : 'bg-white/10'}`}>
                {m.role === 'assistant' ? <Brain className="w-3.5 h-3.5 text-primary" /> : <User className="w-3.5 h-3.5 text-gray-300" />}
              </div>
              <div className={`max-w-lg rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-line ${m.role === 'assistant' ? 'bg-white/5 text-gray-200 rounded-tl-none' : 'bg-primary/20 text-white rounded-tr-none'}`}>
                {m.content}
              </div>
            </div>
          ))}
          {loading && (
            <div className="flex gap-3">
              ...
              <div className="bg-white/5 rounded-2xl rounded-tl-none px-4 py-3 flex gap-1.5">
          ...
          <div className="px-4 pb-3 flex flex-wrap gap-2">
            {suggestions.map(s => (
              <button key={s} onClick={() => send(s)}
                className="text-xs text-gray-400 border border-white/10 rounded-lg px-3 py-1.5 hover:border-primary/30 hover:text-white transition-all bg-white/5">
                {s}
              </button>
            ))}
          </div>
