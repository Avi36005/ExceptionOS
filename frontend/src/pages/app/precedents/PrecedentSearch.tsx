  ...
  return (
    <div className="fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Precedent Library</h1>
          <p className="text-sm text-gray-400 mt-1">{precedents.length} precedents in organizational memory</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate('/app/precedents/ask')} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white text-sm font-medium border border-white/10 transition-colors">
            <Brain className="w-4 h-4" /> Ask AI
          </button>
          <button onClick={() => navigate('/app/precedents/graph')} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white text-sm font-medium border border-white/10 transition-colors">
        ...
        <div className="flex gap-1">
          {categories.map(c => (
            <button key={c} onClick={() => setCategory(c)}
              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${category === c ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}>
              {c === 'all' ? 'All' : c}
            </button>
          ))}
        </div>
        <div className="flex gap-1">
          {[['all', 'All'], ['approved', 'Approved'], ['rejected', 'Rejected']].map(([val, label]) => (
            <button key={val} onClick={() => setDecision(val)}
              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${decision === val ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {filtered.map(p => (
          <div key={p.id} className="bg-dark2 rounded-xl border border-white/10 p-4 hover:border-primary/30 cursor-pointer transition-all"
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {p.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 rounded-md text-xs bg-white/5 text-gray-400 border border-white/5">{tag}</span>
                  ))}
                </div>
              </div>
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium shrink-0 ${p.decision === 'approved' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
