      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Set decision deadlines by category and priority (in hours).</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              {priorities.map(p => (
                <th key={p} className="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider capitalize">{p}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {categories.map(cat => (
              <tr key={cat} className="border-b border-gray-100">
                <td className="py-3 px-4 text-gray-600 font-medium">{cat}</td>
                {priorities.map(p => (
                  <td key={p} className="py-3 px-4 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <input
                        ...
                        className="w-16 bg-gray-50 border border-gray-200 rounded px-2 py-1 text-sm text-gray-900 text-center focus:outline-none focus:border-primary/60 transition-all"
                      />
