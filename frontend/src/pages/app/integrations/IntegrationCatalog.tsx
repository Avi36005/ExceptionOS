    ...
    <div className="fade-in">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Connect ExceptionOS to your existing tools and workflows.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {integrations.map(intg => (
          <div key={intg.id} className={`bg-dark2 rounded-xl border p-5 ${intg.featured ? 'border-primary/30 bg-primary/3' : 'border-white/10'}`}>
            ...
            <p className="text-sm text-gray-400 mb-4">{intg.desc}</p>
            <button
              onClick={() => intg.id === 'openclaw' ? navigate('/app/integrations/openclaw') : undefined}
              className={`w-full py-2 rounded-lg text-xs font-medium transition-colors ${intg.connected ? 'bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10' : 'bg-primary/10 hover:bg-primary/20 text-primary'}`}
            >
