...
const insightCards = [
  { icon: TrendingUp, label: 'Policy Drift', value: '2 policies', desc: 'above threshold', color: 'text-red-600', bg: 'bg-red-500/10', to: '/app/insights/policy-drift' },
  { icon: AlertTriangle, label: 'Repeated Exceptions', value: '8 patterns', desc: 'detected this quarter', color: 'text-yellow-600', bg: 'bg-yellow-500/10', to: '/app/insights/repeated' },
  { icon: Brain, label: 'Memory Health', value: '94%', desc: 'coverage score', color: 'text-green-600', bg: 'bg-green-500/10', to: '/app/insights/memory-health' },
  ...
  return (
    <div className="fade-in space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">AI-powered analysis of your organization's exception patterns and decision quality.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {insightCards.map(c => (
          <button key={c.label} onClick={() => navigate(c.to)} className="bg-white rounded-xl border border-gray-200 p-5 hover:border-primary/30 transition-all text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Policy Compliance by Category</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Approval Rate Trend</h2>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex gap-3 p-3 bg-gray-50 rounded-lg">
              <span className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${r.level === 'high' ? 'bg-red-500' : r.level === 'medium' ? 'bg-yellow-500' : 'bg-green-500'}`} />
