  ...
  return (
    <div className="fade-in max-w-2xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-400 text-sm">Query ExceptionOS hands-free. Ask about exceptions, policies, and decisions.</p>
      </div>
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-white mb-1">{listening ? 'Listening...' : 'Tap to speak'}</p>
          <p className="text-xs text-gray-500">{listening ? 'Speak your query clearly' : 'Ask about any exception, policy, or precedent'}</p>
        </div>

        {/* Transcript */}
        {transcript && (
          <div className="w-full bg-dark2 rounded-xl border border-white/10 p-4 fade-in">
            <div className="text-xs text-gray-500 mb-2">You said:</div>
            <p className="text-sm text-white font-medium mb-4">"{transcript}"</p>
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <Brain className="w-4 h-4 text-primary" />
                <span className="text-xs font-semibold text-primary">Response</span>
              </div>
              <p className="text-sm text-gray-300">Exception EXC-1048 is currently <strong className="text-white">Under Review</strong>. It was submitted by Alex Turner on December 18, 2024 for an emergency software license. The exception is assigned to Sarah Chen for approval. Deadline is December 20, 2024. Would you like me to open it?</p>
              <div className="flex gap-2 mt-3">
                ...
                <button className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-300 text-xs font-medium transition-colors">Dismiss</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                ...
                <div key={cmd} className="flex items-center gap-2 p-3 bg-white/3 rounded-lg border border-white/5">
