...
class ReflectService:
    ...
    async def reflect_on_policy(self, bank_id: str, policy_name: str) -> dict[str, Any]:
        """Reflect on patterns related to a policy."""
        topic = f"What patterns exist in decisions related to policy '{policy_name}'?"
        return await self.hindsight.reflect(bank_id, topic)

    async def reflect_on_entity(self, bank_id: str, entity_name: str) -> dict[str, Any]:
        """Reflect on an entity's exception history."""
        topic = f"What is the history and pattern of exceptions for entity '{entity_name}'?"
        return await self.hindsight.reflect(bank_id, topic)

    async def reflect_on_drift(self, bank_id: str) -> dict[str, Any]:
    ...
    async def reflect_on_topic(self, bank_id: str, topic: str) -> dict[str, Any]:
        """General reflection."""
        return await self.hindsight.reflect(bank_id, topic)
